...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-colors duration-200" 
     x-data="{ certificateOpen: false, selectedCertDose: null, selectedCertDate: '', selectedCertName: '', selectedCertCenter: '', selectedCertRef: '', selectedCertAadhar: '{{ auth()->user()->aadhar_number ? 'XXXX-XXXX-' . substr(auth()->user()->aadhar_number, -4) : 'Not Verified' }}' }">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
        ...
        <div class="bg-white dark:bg-[#151c2c] rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 flex items-center gap-4 transition-all duration-300 hover:shadow-md animate-fade-in-up delay-100">
        ...
        {{-- Card 2: Pending --}}
        <div class="bg-white dark:bg-[#151c2c] rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 flex items-center gap-4 transition-all duration-300 hover:shadow-md animate-fade-in-up delay-200">
        ...
        {{-- Card 3: Completed --}}
        <div class="bg-white dark:bg-[#151c2c] rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 flex items-center gap-4 transition-all duration-300 hover:shadow-md animate-fade-in-up delay-300">
            ...
            <div class="bg-white dark:bg-[#151c2c] rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 animate-fade-in-up delay-100">
                <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
                    <div>
                        ...
                        <p class="text-xs text-slate-405 dark:text-slate-400 mt-0.5">Real-time status of your COVID-19 immunization lifecycle</p>
                    </div>
                    @if($isFullyImmunized)
                ...
                <div class="relative pl-6 border-l-2 border-slate-150 dark:border-slate-800 space-y-8 ml-3">
                    ...
                    <div class="relative">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-450 dark:text-slate-400 mt-0.5">Profile set up on {{ auth()->user()->created_at->format('d M Y') }}.</p>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        @if(auth()->user()->aadhar_verified)
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-450 dark:text-slate-400 mt-0.5">Identity verified using UIDAI biometric registries.</p>
                            </div>
                        @else
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-450 dark:text-slate-400 mt-0.5">Please verify your Aadhaar registry to unlock appointment bookings.</p>
                                </div>
                    ...
                    <div class="relative">
                        @if($dose1Status === 'completed')
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-455 dark:text-slate-400 mt-0.5">
                                        Received **{{ $dose1Appt->vaccine->name }}** on {{ $dose1Appt->appointment_date->format('d M Y') }} at {{ $dose1Appt->center->name }}.
                                    </p>
                                </div>
                                <button type="button" 
                                        @click="selectedCertDose = 1; selectedCertDate = '{{ $dose1Appt->appointment_date->format('d/m/Y') }}'; selectedCertName = '{{ $dose1Appt->vaccine->name }}'; selectedCertCenter = '{{ $dose1Appt->center->name }}'; selectedCertRef = '#TXN-VACC-{{ str_pad($dose1Appt->id, 6, '0', STR_PAD_LEFT) }}'; certificateOpen = true"
                        ...
                        @elseif($dose1Status === 'scheduled')
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-450 dark:text-slate-400 mt-0.5">
                    ...
                    <div class="relative">
                        @if($dose2Status === 'completed')
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                ...
                                <button type="button" 
                                        @click="selectedCertDose = 2; selectedCertDate = '{{ $dose2Appt->appointment_date->format('d/m/Y') }}'; selectedCertName = '{{ $dose2Appt->vaccine->name }}'; selectedCertCenter = '{{ $dose2Appt->center->name }}'; selectedCertRef = '#TXN-VACC-{{ str_pad($dose2Appt->id, 6, '0', STR_PAD_LEFT) }}'; certificateOpen = true"
                        ...
                        @elseif($dose2Status === 'not_booked')
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div>
                                    <h3 class="text-sm font-bold text-slate-700 dark:text-slate-300">Dose 2: Not Booked</h3>
                                    <p class="text-xs text-slate-450 dark:text-slate-400 mt-0.5">
                                        Requires **{{ $activeVaccine->name }}** ({{ $activeVaccine->days_between_doses }} days gap). 
                                        Eligible from: <span class="font-semibold text-slate-750 dark:text-slate-300">{{ $eligibleDate->format('d M Y') }}</span>.
                                    </p>
                                </div>
                                @if($isEligibleNow)
                                ...
                                @else
                                    <span class="sm:self-start bg-slate-100 dark:bg-slate-800 text-slate-450 dark:text-slate-500 px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider border border-slate-200 dark:border-slate-800 select-none shrink-0 cursor-not-allowed">
                                        Soon
                                    </span>
                                @endif
                            </div>
                        ...
                        @else
                            ...
                            <div>
                                <h3 class="text-sm font-semibold text-slate-450 dark:text-slate-500">Dose 2: Locked</h3>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Unlock by completing Dose 1 immunization first.</p>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="relative max-w-2xl w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-3xl p-6 shadow-2xl transition-all transform flex flex-col justify-between"
            ...
            <div id="print-certificate" class="bg-white text-slate-800 p-6 sm:p-8 rounded-2xl border-4 border-emerald-600/30 shadow-inner relative select-none">
                ...
                <div class="text-center border-b border-slate-200 pb-5 mb-6 relative">
                    ...
                    <h2 class="text-xs uppercase tracking-widest font-black text-slate-450 leading-none">Ministry of Health & Family Welfare</h2>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between border-t border-slate-200 pt-6 gap-6 relative">
                    ...
                    <div class="w-24 h-24 bg-emerald-50 rounded-xl p-2 flex items-center justify-center shrink-0 border-2 border-emerald-500/20 select-none shadow-sm">
                        <svg class="w-full h-full text-emerald-700" viewBox="0 0 100 100" fill="currentColor">
                            {{-- Nested boxes to look like a high-end QR code --}}
                            ...
                            <rect x="55" y="12" width="15" height="8" />
                            <rect x="35" y="25" width="30" height="5" />
                            <rect x="75" y="35" width="10" height="15" />
                            <rect x="15" y="45" width="10" height="10" />
                            <rect x="35" y="45" width="15" height="20" />
                            <rect x="60" y="55" width="15" height="15" />
            ...
            <div class="flex gap-3 mt-6 border-t border-slate-100 dark:border-slate-800 pt-4 no-print">
                ...
                <button type="button" onclick="window.print()"
                        class="flex-1 py-3 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white rounded-xl text-sm font-bold shadow-md shadow-blue-500/15 hover:shadow-blue-500/25 transition-all flex items-center justify-center gap-2">
